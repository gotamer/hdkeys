...
func flag() {
	...
	for no, arg := range os.Args {
		...
		case "-from":
			Debug.Println("In case -from")
			flag_from = getNextArgAsInt(no, 0)
			continue

		case "-to":
			Debug.Println("In case -to")
			flag_to = getNextArgAsInt(no, 10)
			continue
		}
...
- hdkeys recover can recover a BIP39 passwords
...
COMMANDS
	...
	recover
		Recover will run a wordlist against a Mnemonic seed phrase to recover the BIP39 password.
		COMMAND OPTIONS
			-from [int] (default = 0)
				Public key index number from.
			-to [int] (default = 10)
				Public key index number to.
